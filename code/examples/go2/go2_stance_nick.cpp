...
int main() {
    ...
    q0[2] = 0.3689268220122;  // base height ~30cm

    // Hip, thigh, calf angles for a standing pose
    // FL
    q0[7]  =  0.0;    // FL_hip
    q0[8]  =  M_PI/6.;    // FL_thigh
    q0[9]  = -M_PI/3.;    // FL_calf
    // FR
    q0[10] =  0.0;    // FR_hip
    q0[11] =  M_PI/6.;     // FR_thigh
    q0[12] = -M_PI/3.;    // FR_calf
    // RL
    q0[13] =  0.0;    // RL_hip
    q0[14] =  M_PI/6.;     // RL_thigh
    q0[15] = -M_PI/3.;    // RL_calf
    // RR
    q0[16] =  0.0;    // RR_hip
    q0[17] =  M_PI/6.;     // RR_thigh
    q0[18] = -M_PI/3.;    // RR_calf
    ...
    for (int k = 0; k < N-1; k++) {
        ...
        // Costs
        node.add_cost(std::make_shared<ConfigurationCost>(q0, 1));
        node.add_cost(std::make_shared<VelocityCost>(1e-3));
        node.add_cost(std::make_shared<AccelerationCost>(1e-4));
