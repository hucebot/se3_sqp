...
int main() {
    ...
    scheduler.define_contact("none", {});


    scheduler.addPhase({"all"}, 1., "jump");
    scheduler.addPhase({"none"}, 0.2, "jump");
    scheduler.addPhase({"all"}, .8, "jump");

    // Generate contact sequence for 2 full gait cycles
    auto contact_sequence = scheduler.getSequence(dt, "jump");
    ...
    // ── Build OCP ──
    OCP ocp(N);
    auto seq_it = contact_sequence.begin();
    ...
    // ── Save trajectory ──
    ocp.save_trajectory("/workspace/code/resources/trajectories/go2_jump.json",
                        dt, urdf_path);
