...
bool SQPSolver::ls_merit() {
    ...
    const double phi_alpha =  _candidate_cost + mu * (_candidate_viol);
    const double phi_0     =  _nominal_cost + mu * (_nominal_viol);
    ...
    for (int k = 0; k < _N; ++k) {
        ...
        // dphi += mu * node.get_defect_grad_x().dot(_dx[k]);  // BUG1 test: defect grad excluded
        dphi += mu * node.get_violation_grad_x().dot(_dx[k]);

        if (k < _Nu) {
            dphi += node.get_cost_grad_u().dot(_du[k]);
            // dphi += mu * node.get_defect_grad_u().dot(_du[k]);  // BUG1 test: defect grad excluded
